...
class Api_test():
    ...
    def __init__(self, s):
        self.s = s
        self.s.verify = False
        self.url = "https://cloud.qccvas.com/gateway-nginx"
    ...
    def test_read(self, zipname):
        try:
            cfg = ConfigParser()
            cfg.read('./test_files/cloud_version.ini')
            cfg.sections()
            ...
            iniset = open(r'./test_files/cloud_version.ini', 'w')
            cfg.write(iniset)
            # insert data
...
if __name__ == '__main__':
    s = requests.Session()
    a = Api_test(s)
    a.login(username="chuweiqi",password=123456)
    # print(s.headers)
    #a.qcc_002()
    a.app_update()
